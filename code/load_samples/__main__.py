...
def getter(taxon : str, index : str, ribo_dir : str) -> None:
    """ Run a subprocess to fetch the genome corresponding to the accession number. """
    subprocess.run(
...
def load_taxid(taxon_file : str, id=False) -> Dict[str, str]:
...
if __name__ == '__main__':
    ...
    parser.add_argument("samples", help="The name of the input file (must be in `input/samples_list/`)")
    parser.add_argument("--id", help="Set to true if the file contains the accession ids instead of the taxon names", action='store_true')
    args = parser.parse_args()

    root_dir = os.path.abspath(os.path.join(__file__, "..", "..", ".."))
    taxon_file = args.samples
    taxon_file = os.path.join(root_dir, "input", "samples_list", taxon_file)

    taxon_basename = os.path.basename(taxon_file).split('.')[0]
    out_dir = os.path.join(root_dir, "input", "sequence_db", taxon_basename)
    ribo_dir = os.path.join(root_dir, "input", "ribosome_db", taxon_basename)
